      ...
      <div className="relative h-72 md:h-96 overflow-hidden" style={{ background: business.colorDark }}>
        ...
        <div className="absolute inset-0 flex flex-col justify-end px-8 md:px-20 pb-10">
          ...
          >
            <button
              onClick={() => navigate("/")}
              className="text-green-300 text-sm font-semibold mb-4 hover:text-white transition-colors flex items-center gap-1"
            >
              ← Back
            </button>
            <span
            ...
            <p className="text-green-200 text-lg mt-2">{service.tagline}</p>
          </motion.div>
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-3 gap-10">
          <div className="md:col-span-2 space-y-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}>
              <h2 className="oswald text-2xl font-bold text-gray-900 mb-3">About This Service</h2>
              <p className="text-gray-600 leading-relaxed text-base">{service.description}</p>
            </motion.div>

            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }}>
              <h3 className="oswald text-xl font-bold text-gray-900 mb-4">What's Included</h3>
          ...
          >
            <div className="text-2xl font-bold oswald mb-1" style={{ color: business.colorPrimary }}>
              {service.price}
            </div>
            <p className="text-gray-500 text-sm mb-6">No obligation. We'll get back to you same day.</p>
            <a
